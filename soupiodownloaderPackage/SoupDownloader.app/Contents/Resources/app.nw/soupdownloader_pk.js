...
function soupliner(setup){
	var count = 0,
		souppiccounter = 0,
		soupRSSfilpath = setup[2], //'./soup_falk_2013-01-03.rss', // change to reflect yours
		stream = fs.createReadStream(soupRSSfilpath),
		options = {},
		lastTitle = '',
		metaData = setup[1]; //true; // if you want a metadatafile to each image from the soup info

	var parralelDLs = setup[0]; //2; // how many pictures will be downloaded in parralel - adjust to soup speed of the day and you connection speed
	var path = setup[3]; //'./soupImages/'; // change to reflect yours 
	...
	if (!fs.existsSync(path)) {
		$('#consolelog').text('Creating Directory: ' + path);
		        			...
		        			fs.writeFile(fullpath + '.' + fileext, imagedata, 'binary', function(err){
		            			...
		            			//write MetaData Text File
			            			fs.writeFile(fullpath + '.txt', metadata.stringedSoup, 'utf8', function(err){
			            				...
			            				$('#consolelog').text('File ' + fullpath + ' and Metadata saved.');
			            				console.log('File ' + fullpath + ' and Metadata saved.');
			            				callback();
		            				});
		            			} else {
		            				$('#consolelog').text('File ' + fullpath + ' saved.');
